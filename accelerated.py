...
def quadratic_subpixel_peak(array):
    ...
    smoothed = gaussian_smooth_time(array, 20*(fps/nmps)/frame_factor) # nms to smooth over * frames per nm after decimation = frames to smooth over

    # take product over color channels of cross correlations to take advantage of different chirp frequencies in each channel
...
if __name__ == "__main__":
    ...
    video_array_BGR = load_video(args.video_file, crop_region, rgb=True)
    ...
    # take the average of the color channel heightmaps. there are smarter ways to combine...
    ...
    fig = go.Figure(data=[go.Surface(z=max_indices,  colorscale='earth')])
